...
fn get_between_coords(pipe: &VentPipe) -> Vec<Coordinate> {
    ...
    let mut working_cord = [pipe[0][0] as i64, pipe[0][1] as i64];
    let target_cord = [pipe[1][0] as i64, pipe[1][1] as i64];
    let mut offset: [i64; 2] = [0; 2];
    for offset_idx in 0..2 {
        if pipe[1][offset_idx] > pipe[0][offset_idx] {
            offset[offset_idx] = 1;
        } else if pipe[1][offset_idx] < pipe[0][offset_idx] {
            offset[offset_idx] = -1;
        }
    }

    coords.push([working_cord[0] as usize, working_cord[1] as usize]);
...
fn main() {
    ...
    // println!("{:?}", get_between_coords(&[[5, 5], [8, 2]]));
}
