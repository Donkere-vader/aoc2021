...
fn main() {
    ...
    let mut boards_yet_to_win: Vec<usize> = (0..boards.len()).collect();
    ...
    for num in numbers.iter() {
        ...
        for (idx, board) in boards.iter().enumerate() {
            if !boards_yet_to_win.contains(&idx) { continue }
            ...
            if won {
                last_num = *num;
                for (idx, value) in boards_yet_to_win.iter().enumerate() {
                    if *value == winning_board_idx {
                        boards_yet_to_win.remove(idx);
                        break;
                    }
                }
                if boards_yet_to_win.len() == 0 {
                    break;
                }
            }
        }
