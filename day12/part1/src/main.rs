...
fn get_paths(map: &Map, node: &String) -> Vec<Vec<String>> {
    ...
    traverse(map, node, &mut path, &mut paths);

    paths
}

fn traverse(map: &Map, node: &String, path: &mut Vec<String>, paths: &mut Vec<Vec<String>>) {
    ...
    if *node == String::from("end") {
        ...
        for cave in entry.iter() {
            if cave.to_lowercase() == *cave && path.contains(cave) { continue };
            path.push(cave.to_string());
            traverse(map, cave, path, paths);
